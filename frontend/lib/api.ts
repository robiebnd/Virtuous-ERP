...
export const financeApi = {
 ...
 depreciateAsset:(id:string,months=1)=>api<any>(`/api/finance/fixed-assets/${id}/depreciate?months=${months}`,{method:"POST"}),
  ...
  treasuryRiskLimits:()=>list<any>("/api/finance/treasury/risk-limits"),
  createTreasuryRiskLimit:(body:any)=>api<any>("/api/finance/treasury/risk-limits",{method:"POST",body:JSON.stringify(body)}),
  treasuryInstruments:()=>list<any>("/api/finance/treasury/instruments"),
  createTreasuryInstrument:(body:any)=>api<any>("/api/finance/treasury/instruments",{method:"POST",body:JSON.stringify(body)}),
  valueTreasuryInstrument:(body:any)=>api<any>("/api/finance/treasury/instruments/valuation",{method:"POST",body:JSON.stringify(body)}),
  ...
  consolidationJournals:(runId:string)=>list<any>(`/api/finance/group-reporting/runs/${runId}/journals`),
  consolidationAudit:(groupId:string)=>list<any>(`/api/finance/group-reporting/groups/${groupId}/audit`),
  consolidationRunAudit:(runId:string)=>list<any>(`/api/finance/group-reporting/runs/${runId}/audit`),
  postConsolidationJournal:(body:any)=>api<any>("/api/finance/group-reporting/journals",{method:"POST",body:JSON.stringify(body)}),
  ...
  taxReport:(companyCode:string,periodStart:string,periodEnd:string)=>api<any>(`/api/finance/group-reporting/tax-report?companyCode=${encodeURIComponent(companyCode)}&periodStart=${periodStart}&periodEnd=${periodEnd}`),
  taxFilings:(companyCode:string)=>list<any>(`/api/finance/group-reporting/tax-filings?companyCode=${encodeURIComponent(companyCode)}`),
  prepareTaxFiling:(body:any)=>api<any>("/api/finance/group-reporting/tax-filings/prepare",{method:"POST",body:JSON.stringify(body)}),
  fileTaxFiling:(body:any)=>api<any>("/api/finance/group-reporting/tax-filings/file",{method:"POST",body:JSON.stringify(body)}),
