...
export default function NewSalesOrder(){
  const [form,setForm]=useState({customerCode:"",salesOrganization:"1000",distributionChannel:"10",division:"00",currency:"USD",remarks:""});
  ...
  const submit=async()=>{setSaving(true);setMessage("");try{const result=await orderToCashApi.createSalesOrder({customerCode:form.customerCode,salesOrganization:form.salesOrganization,distributionChannel:form.distributionChannel,division:form.division,currency:form.currency,remarks:form.remarks,items:items.map(i=>({materialCode:i.materialCode,quantity:Number(i.quantity),unitPrice:Number(i.unitPrice)}))});setMessage(`Sales order ${result.orderNumber||""} created with status ${result.status||"DRAFT"}.`);}catch(e){setMessage(e instanceof Error?e.message:"Unable to create sales order.");}finally{setSaving(false);}};
