...
export default function NewVendorPayment(){
  ...
  useEffect(()=>{procurementApi.vendorInvoices().then(setInvoices).catch(()=>setInvoices([]));},[]);
  ...
  return <div className="content"><div className="document-head"><div><div className="eyebrow">PROCUREMENT / ACCOUNTS PAYABLE</div><h1>Create Vendor Payment</h1><p>Create a payment proposal only against a matched or posted vendor invoice.</p></div><div className="actions"><Link className="btn" href="/procurement/vendor-payments">Cancel</Link><button className="btn primary" disabled={saving||!form.vendorInvoiceId||!form.amount} onClick={submit}>{saving?"Creating…":"Create Payment"}</button></div></div>
    ...
    <section className="card form-card"><div className="section-title">Payment Details</div><div className="form-grid">
      <div className="form-field"><label>Vendor Invoice *</label><select className="form-input" value={form.vendorInvoiceId} onChange={e=>{const id=e.target.value;const invoice=invoices.find(x=>x.id===id);set("vendorInvoiceId",id);if(invoice){set("amount",String(invoice.totalAmount||invoice.total||""));set("currency",invoice.currency||"USD");}}}><option value="">Select matched vendor invoice</option>{invoices.filter(x=>["MATCHED","POSTED"].includes(x.status)).map(x=><option key={x.id} value={x.id}>{x.invoiceNumber||x.number||"Invoice"} · {x.supplier?.name||x.supplierName||"Supplier"} · {x.currency||"USD"} {Number(x.totalAmount||x.total||0).toFixed(2)}</option>)}</select></div>
