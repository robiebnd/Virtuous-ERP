...
export default function TaxAccountingPage(){
 ...
 const load=async(c=company)=>{try{const [co,tc,tp]=await Promise.all([financeApi.companyCodes(),financeApi.taxCodes(),financeApi.taxPostings(c)]);setCompanies(co);setCodes(tc.filter((x:any)=>!x.companyCode||x.companyCode===c));setPostings(tp);setError("")}catch(e){setError(e instanceof Error?e.message:"Unable to load tax accounting.")}};
 const runReport=async()=>{try{setReport(await financeApi.taxReport(company,reportStart,reportEnd));setError("")}catch(e){setError(e instanceof Error?e.message:"Unable to load tax report.")}};
 useEffect(()=>{load()},[]);
 const saveCode=async()=>{setBusy(true);try{await financeApi.saveTaxCode({...codeForm,rate:Number(codeForm.rate),recoverablePercent:Number((codeForm as any).recoverablePercent??100)});await load(company);setError("")}catch(e){setError(e instanceof Error?e.message:"Unable to save tax code.")}finally{setBusy(false)}};
 ...
 return <main className="content">
  ...
  <div className="grid" style={{gridTemplateColumns:"1fr 1fr",marginBottom:18}}>
   <section className="card form-card"><div className="section-title">Tax Code Setup</div><div className="form-grid">
    ...
    <div className="form-field"><label>Tax Type</label><input className="form-input" value={(codeForm as any).taxType||"VAT"} onChange={e=>setCodeForm({...codeForm,taxType:e.target.value} as any)}/></div>
    <div className="form-field"><label>Recoverable %</label><input className="form-input" type="number" value={(codeForm as any).recoverablePercent??"100"} onChange={e=>setCodeForm({...codeForm,recoverablePercent:e.target.value} as any)}/></div>
  ...
  <section className="card" style={{marginBottom:18}}><div className="table-caption"><strong>Tax Report</strong><span>Period summary and filing base</span></div><div className="toolbar"><div className="filter-field"><label>From</label><input className="filter" type="date" value={reportStart} onChange={e=>setReportStart(e.target.value)}/></div><div className="filter-field"><label>To</label><input className="filter" type="date" value={reportEnd} onChange={e=>setReportEnd(e.target.value)}/></div><div className="filter-field"><label>&nbsp;</label><button className="btn primary" onClick={runReport}>Run Tax Report</button></div></div>{report&&<div className="table-wrap"><table className="table"><thead><tr><th>Output Tax</th><th>Input Tax</th><th>Recoverable Input</th><th>Net Tax</th></tr></thead><tbody><tr><td>{Number(report.outputTax||0).toFixed(2)}</td><td>{Number(report.inputTax||0).toFixed(2)}</td><td>{Number(report.recoverableInputTax||0).toFixed(2)}</td><td><strong>{Number(report.netTax||0).toFixed(2)}</strong></td></tr></tbody></table></div>}</section>
