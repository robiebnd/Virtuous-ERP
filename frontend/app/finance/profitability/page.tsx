...
export default function ProfitabilityPage(){
 ...
 return <main className="content">
  ...
  <section className="card"><div className="table-caption"><strong>Contribution Margin Report</strong><span>{company} · FY {year} / P{period}</span></div><div className="table-wrap"><table className="table"><thead><tr><th>Segment</th><th>Revenue</th><th>Cost</th><th>Contribution Margin</th><th>Margin %</th></tr></thead><tbody>{report.map(x=>{const margin=x.revenue?Number(x.contributionMargin)/Number(x.revenue)*100:0;return <tr key={x.segmentId}><td><strong>{x.segmentCode}</strong> — {x.segmentName}</td><td>{Number(x.revenue||0).toFixed(2)}</td><td>{Number(x.cost||0).toFixed(2)}</td><td>{Number(x.contributionMargin||0).toFixed(2)}</td><td>{margin.toFixed(1)}%</td></tr>})}</tbody></table></div></section>
 </main>
}