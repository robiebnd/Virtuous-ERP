...
export default function JournalEntryPage(){
 ...
 useEffect(()=>{financeApi.glAccounts().then(setAccounts).catch(()=>setAccounts([]));financeApi.profitabilitySegments("ZW01").then(setSegments).catch(()=>setSegments([]));},[]);
 const setLine=(i:number,key:keyof Line,value:string)=>setLines(v=>v.map((x,n)=>n===i?{...x,[key]:value}:x));
 const addLine=()=>setLines(v=>[...v,{accountCode:"",debit:"0.00",credit:"0.00",costCenter:"",profitCenter:"",lineText:"",profitabilitySegmentId:""}]);
 ...
 return <div className="content">
  ...
  <section className="card form-card"><div className="section-title"><span>Journal Lines</span><button className="btn" onClick={addLine}>Add Line</button></div>
   <div className="table-wrap"><table className="table"><thead><tr><th>G/L Account</th><th>Debit</th><th>Credit</th><th>Cost Center</th><th>Profit Center</th><th>CO-PA Segment</th><th>Text</th><th></th></tr></thead><tbody>
    {lines.map((x,i)=><tr key={i}><td><select className="form-input" value={x.accountCode} onChange={e=>setLine(i,"accountCode",e.target.value)}><option value="">Select account</option>{accounts.map(a=><option key={a.accountCode} value={a.accountCode}>{a.accountCode} — {a.accountName}</option>)}</select></td><td><input className="form-input" type="number" min="0" step="0.01" value={x.debit} onChange={e=>setLine(i,"debit",e.target.value)}/></td><td><input className="form-input" type="number" min="0" step="0.01" value={x.credit} onChange={e=>setLine(i,"credit",e.target.value)}/></td><td><input className="form-input" value={x.costCenter} onChange={e=>setLine(i,"costCenter",e.target.value)} placeholder="Optional"/></td><td><input className="form-input" value={x.profitCenter} onChange={e=>setLine(i,"profitCenter",e.target.value)} placeholder="Optional"/></td><td><select className="form-input" value={x.profitabilitySegmentId} onChange={e=>setLine(i,"profitabilitySegmentId",e.target.value)}><option value="">None</option>{segments.map(s=><option key={s.id} value={s.id}>{s.segmentCode}</option>)}</select></td><td><input className="form-input" value={x.lineText} onChange={e=>setLine(i,"lineText",e.target.value)}/></td><td><button className="btn" disabled={lines.length<=2} onClick={()=>removeLine(i)}>Remove</button></td></tr>)}
   </tbody></table></div>
