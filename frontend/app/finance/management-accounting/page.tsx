...
export default function ManagementAccountingPage(){
 ...
 return <main className="content">
  ...
  <section className="card form-card"><div className="section-title">Reporting Period</div><div className="form-grid"><div className="form-field"><label>Fiscal Year</label><input className="form-input" type="number" value={year} onChange={e=>setYear(Number(e.target.value))}/></div></div></section>
  <section className="card"><div className="section-title">Cost Centre Actual vs Budget</div>{loading?<div className="empty">Loading…</div>:<div className="table-wrap"><table><thead><tr><th>Cost Centre</th><th>Actual</th><th>Budget</th><th>Variance</th><th>Status</th></tr></thead><tbody>{centers.map(x=><tr key={x.costCenterCode}><td><b>{x.costCenterCode}</b></td><td>{money(x.actualAmount)}</td><td>{money(x.budgetAmount)}</td><td>{money(x.variance)}</td><td><span className={Number(x.variance)<0?"status pending":"status approved"}>{Number(x.variance)<0?"OVER BUDGET":"WITHIN BUDGET"}</span></td></tr>)}</tbody></table></div>}</section>
