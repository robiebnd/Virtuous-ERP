...
public class TreasuryService {
    ...
    public TreasuryRiskLimit saveRiskLimit(com.digipals.wms.finance.dto.TreasuryRiskLimitRequest r){TreasuryRiskLimit x=riskLimits.findByLimitCode(r.limitCode().trim()).orElseGet(TreasuryRiskLimit::new);x.setLimitCode(r.limitCode().trim());x.setCounterparty(r.counterparty());x.setCurrency(r.currency()==null?null:currency(r.currency()));x.setInstrumentType(r.instrumentType()==null?null:r.instrumentType().trim().toUpperCase());x.setLimitAmount(money(r.limitAmount()));x.setActive(true);return riskLimits.save(x);}
 public List<TreasuryRiskLimit> riskLimits(){return riskLimits.findAll();}
