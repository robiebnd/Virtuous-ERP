...
public class BillingDocumentServiceImpl implements BillingDocumentService {
    ...
    public BillingDocument create(CreateBillingRequest request) {
        ...
        OutboundDelivery delivery = outboundDeliveryRepository.findById(request.outboundDeliveryId()).orElseThrow(() -> new ResourceNotFoundException("Outbound delivery not found: " + request.outboundDeliveryId()));
        ...
        LocalDateTime billingDate = LocalDateTime.now(); LocalDateTime dueDate = request.dueDate() == null ? billingDate.plusDays(30) : request.dueDate();
        if (dueDate.isBefore(billingDate)) throw new InvalidWorkflowException("Billing due date cannot be before billing date.");
        BillingDocument billing = BillingDocument.builder().billingNumber(generateBillingNumber()).outboundDelivery(delivery).customerCode(delivery.getCustomerCode()).billingType("F2").currency(request.currency().trim().toUpperCase()).status(BillingStatus.DRAFT).billingDate(billingDate).dueDate(dueDate).remarks(delivery.getRemarks()).totalAmount(BigDecimal.ZERO).build();
        BigDecimal total = BigDecimal.ZERO; int itemNumber = 10;
    ...
    public BillingDocument post(UUID id) {
        ...
        billing.setStatus(BillingStatus.POSTED); BillingDocument saved = billingDocumentRepository.save(billing);
    ...
    private SalesOrderItem findSalesOrderItem(OutboundDelivery delivery, OutboundDeliveryItem deliveryItem) { return delivery.getSalesOrder().getItems().stream().filter(item -> item.getItemNumber().equals(deliveryItem.getItemNumber()) || item.getMaterialCode().equalsIgnoreCase(deliveryItem.getMaterialCode())).findFirst().orElseThrow(() -> new InvalidWorkflowException("Sales order item not found for delivery item: " + deliveryItem.getItemNumber())); }
