...
public class DocumentFlowServiceImpl implements DocumentFlowService {
    ...
    @Override
    public DocumentFlowResponse getBySalesOrderId(UUID salesOrderId) {
    ...
    @Override
    public DocumentFlowResponse getByDeliveryId(UUID deliveryId) {
        OutboundDelivery delivery = outboundDeliveryRepository.findById(deliveryId)
                .orElseThrow(() -> new ResourceNotFoundException("Outbound delivery not found: " + deliveryId));
        return buildFlow(delivery.getSalesOrder());
    }

    @Override
    public DocumentFlowResponse getByBillingDocumentId(UUID billingDocumentId) {
        BillingDocument billing = billingDocumentRepository.findById(billingDocumentId)
                .orElseThrow(() -> new ResourceNotFoundException("Billing document not found: " + billingDocumentId));
    ...
    private DocumentFlowResponse buildFlow(SalesOrder salesOrder) {
        ...
        List<OutboundDelivery> deliveries = outboundDeliveryRepository
                .findBySalesOrderIdOrderByCreatedAtDesc(salesOrder.getId());
        ...
        for (OutboundDelivery delivery : deliveries) {
            ...
            billingDocumentRepository.findByOutboundDeliveryId(delivery.getId())
                    .ifPresent(billing -> flow.add(entry(
                            "BILLING_DOCUMENT",
                            billing.getId(),
        ...
        return DocumentFlowResponse.builder()
                .rootDocumentType("SALES_ORDER")
                .rootDocumentId(salesOrder.getId())
                .rootDocumentNumber(salesOrder.getOrderNumber())
                .customerCode(salesOrder.getCustomerCode())
                .flow(flow)
                .build();
    }

    private DocumentFlowEntryResponse entry(
            String type,
            UUID id,
            String number,
            String status,
            String relationship) {
        return DocumentFlowEntryResponse.builder()
                .documentType(type)
                .documentId(id)
