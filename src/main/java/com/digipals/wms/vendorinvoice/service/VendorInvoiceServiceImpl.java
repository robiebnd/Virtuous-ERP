...
public class VendorInvoiceServiceImpl implements VendorInvoiceService {
    ...
    public VendorInvoiceResponse create(CreateVendorInvoiceRequest request) {
        ...
        if (po.getSupplier() == null || gr.getPurchaseOrder() == null ||
                !po.getId().equals(gr.getPurchaseOrder().getId())) {
            throw new InvalidWorkflowException("Goods Receipt must belong to the supplied Purchase Order.");
        }
        if (gr.getStatus() != ReceiptStatus.APPROVED) {
            throw new InvalidWorkflowException("Only an APPROVED Goods Receipt can be invoiced.");
        }
        ...
        VendorInvoice invoice = VendorInvoice.builder()
                .invoiceNumber(documentNumberService.next(DocumentType.VENDOR_INVOICE))
                .supplier(po.getSupplier())
                .purchaseOrder(po)
                .goodsReceipt(gr)
                .supplierInvoiceNumber(request.getSupplierInvoiceNumber().trim())
                .invoiceDate(request.getInvoiceDate() == null ? LocalDateTime.now() : request.getInvoiceDate())
        ...
        Map<UUID, List<GoodsReceiptLine>> grLines = gr.getLines().stream()
                .collect(Collectors.groupingBy(line -> line.getPurchaseOrderLine().getId()));

        for (CreateVendorInvoiceRequest.Line requestLine : request.getLines()) {
            if (!seenPoLines.add(requestLine.getPurchaseOrderLineId())) {
                throw new InvalidWorkflowException("A Purchase Order line may only occur once on an invoice.");
            }
            PurchaseOrderLine poLine = purchaseOrderLineRepository.findById(requestLine.getPurchaseOrderLineId())
                    .orElseThrow(() -> new ResourceNotFoundException("Purchase Order line not found."));
            if (!poLine.getPurchaseOrder().getId().equals(po.getId())) {
                throw new InvalidWorkflowException("Invoice line does not belong to the Purchase Order.");
            }
    ...
    public VendorInvoiceResponse match(UUID id) {
        ...
        for (VendorInvoiceLine line : invoice.getLines()) {
            ...
            line.setQuantityVariance(quantityVariance);
            line.setPriceVariance(priceVariance);

            BigDecimal qtyTolerance = poQty.multiply(nvl(po.getOverdeliveryTolerancePercent()))
                    .divide(new BigDecimal("100"), 6, RoundingMode.HALF_UP).add(DEFAULT_QUANTITY_TOLERANCE);
            BigDecimal priceTolerance = poPrice.abs().multiply(DEFAULT_PRICE_TOLERANCE_PERCENT)
            ...
            if (lineReasons.isEmpty()) {
                line.setMatchStatus(VendorInvoiceMatchStatus.MATCHED);
                line.setBlockReason(null);
            } else {
                blocked = true;
                line.setMatchStatus(VendorInvoiceMatchStatus.BLOCKED);
                line.setBlockReason(String.join("; ", lineReasons));
                reasons.add(lineReasons.stream().collect(Collectors.joining(", ")));
            }
        }
    ...
    @Override @Transactional(readOnly=true)
    public VendorInvoiceResponse findByNumber(String invoiceNumber) {
        return toResponse(invoiceRepository.findByInvoiceNumber(invoiceNumber).orElseThrow(() -> new ResourceNotFoundException("Vendor Invoice not found.")));
    }
    ...
    private VendorInvoice getInvoice(UUID id) {
        return invoiceRepository.findWithLinesById(id).orElseThrow(() -> new ResourceNotFoundException("Vendor Invoice not found."));
    }

    private GoodsReceiptLine resolveReceiptLine(UUID requestedId, PurchaseOrderLine poLine, List<GoodsReceiptLine> candidates) {
        if (requestedId != null) {
            GoodsReceiptLine line = goodsReceiptLineRepository.findById(requestedId)
                    .orElseThrow(() -> new ResourceNotFoundException("Goods Receipt line not found."));
            if (!line.getPurchaseOrderLine().getId().equals(poLine.getId())) throw new InvalidWorkflowException("Goods Receipt line does not match Purchase Order line.");
            return line;
        }
    ...
    private VendorInvoiceResponse toResponse(VendorInvoice i) {
        return VendorInvoiceResponse.builder().id(i.getId()).invoiceNumber(i.getInvoiceNumber()).supplierInvoiceNumber(i.getSupplierInvoiceNumber())
                ...
                .purchaseOrderId(i.getPurchaseOrder().getId()).purchaseOrderNumber(i.getPurchaseOrder().getPoNumber())
                .goodsReceiptId(i.getGoodsReceipt().getId()).goodsReceiptNumber(i.getGoodsReceipt().getGrnNumber()).invoiceDate(i.getInvoiceDate())
                .currency(i.getCurrency()).subtotal(i.getSubtotal()).taxAmount(i.getTaxAmount()).totalAmount(i.getTotalAmount())
                .status(i.getStatus()).matchStatus(i.getMatchStatus()).blockReason(i.getBlockReason())
                .lines(i.getLines().stream().map(l -> VendorInvoiceResponse.Line.builder().id(l.getId()).purchaseOrderLineId(l.getPurchaseOrderLine().getId())
                        .goodsReceiptLineId(l.getGoodsReceiptLine()==null?null:l.getGoodsReceiptLine().getId()).productId(l.getProduct().getId()).sku(l.getProduct().getSku())
                        .invoicedQuantity(l.getInvoicedQuantity()).invoiceUnitPrice(l.getInvoiceUnitPrice()).lineTotal(l.getLineTotal())
                        .quantityVariance(l.getQuantityVariance()).priceVariance(l.getPriceVariance()).matchStatus(l.getMatchStatus()).blockReason(l.getBlockReason()).build()).toList()).build();
    }
}
