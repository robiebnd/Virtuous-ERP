...
public class GoodsReceiptServiceImpl implements GoodsReceiptService {
    ...
    public GoodsReceiptResponse create(CreateGoodsReceiptRequest request) {
        ...
        if (purchaseOrder.getStatus() != PurchaseOrderStatus.APPROVED && purchaseOrder.getStatus() != PurchaseOrderStatus.PARTIALLY_RECEIVED) throw new InvalidWorkflowException("Only approved or partially received Purchase Orders can be received.");
    ...
    public GoodsReceiptResponse update(UUID id, UpdateGoodsReceiptRequest request) {
        GoodsReceipt goodsReceipt = getGoodsReceiptWithLines(id);
        ...
        if (request.getLines() != null) {
            ...
            for (UpdateGoodsReceiptRequest.LineQuantityRequest requestedLine : request.getLines()) {
                if (requestedLine.getSku() == null || requestedLine.getSku().isBlank()) throw new InvalidWorkflowException("SKU is required when updating a Goods Receipt line.");
                GoodsReceiptLine receiptLine = goodsReceipt.getLines().stream().filter(line -> line.getProduct() != null && requestedLine.getSku().trim().equalsIgnoreCase(line.getProduct().getSku())).findFirst().orElseThrow(() -> new ResourceNotFoundException("Goods Receipt line not found for SKU: " + requestedLine.getSku()));
                BigDecimal received = nullSafe(requestedLine.getReceivedQuantity());
                BigDecimal accepted = nullSafe(requestedLine.getAcceptedQuantity());
                BigDecimal rejected = nullSafe(requestedLine.getRejectedQuantity());
    ...
    public GoodsReceiptResponse approve(UUID id) {
        GoodsReceipt goodsReceipt = getGoodsReceiptWithLines(id);
        if (goodsReceipt.getStatus() != ReceiptStatus.DRAFT) throw new InvalidWorkflowException("Only draft Goods Receipts can be approved.");
        ...
        for (GoodsReceiptLine receiptLine : receiptLines) {
            ...
            if (newReceivedQuantity.compareTo(purchaseOrderLine.getQuantity()) > 0) throw new InvalidWorkflowException("Accepted quantity would exceed the outstanding Purchase Order quantity for product " + receiptLine.getProduct().getSku() + ".");
    ...
    private void validateReceiptLine(GoodsReceiptLine receiptLine) {
        if (receiptLine.getPurchaseOrderLine() == null) throw new InvalidWorkflowException("Goods Receipt Line is not linked to a Purchase Order Line.");
        ...
        BigDecimal ordered = nullSafe(receiptLine.getOrderedQuantity());
        if (accepted.add(rejected).compareTo(received) != 0) throw new InvalidWorkflowException("Accepted quantity + rejected quantity must equal received quantity.");
        if (received.compareTo(ordered) > 0) throw new InvalidWorkflowException("Received quantity cannot exceed the outstanding Purchase Order quantity.");
        if (accepted.compareTo(received) > 0) throw new InvalidWorkflowException("Accepted quantity cannot exceed received quantity.");
    }
    ...
    private void updatePurchaseOrderReceiptStatus(PurchaseOrder purchaseOrder) {
        List<PurchaseOrderLine> purchaseOrderLines = purchaseOrderLineRepository.findByPurchaseOrderId(purchaseOrder.getId());
        boolean fullyReceived = !purchaseOrderLines.isEmpty() && purchaseOrderLines.stream().allMatch(line -> nullSafe(line.getOutstandingQuantity()).compareTo(BigDecimal.ZERO) == 0);
        purchaseOrder.setStatus(fullyReceived ? PurchaseOrderStatus.RECEIVED : PurchaseOrderStatus.PARTIALLY_RECEIVED);
        purchaseOrderRepository.save(purchaseOrder);
    }

    @Override @Transactional(readOnly = true)
    public List<GoodsReceiptResponse> findAll() { return repository.findAll().stream().map(GoodsReceiptMapper::toResponse).toList(); }
    @Override @Transactional(readOnly = true)
    public GoodsReceiptResponse findById(UUID id) { return GoodsReceiptMapper.toResponse(getGoodsReceiptWithLines(id)); }
    @Override @Transactional(readOnly = true)
    public GoodsReceiptResponse findByNumber(String grnNumber) { return GoodsReceiptMapper.toResponse(getGoodsReceiptByNumberWithLines(grnNumber)); }
    @Override @Transactional(readOnly = true)
    public List<GoodsReceiptResponse> findByPurchaseOrder(UUID purchaseOrderId) { return repository.findByPurchaseOrderId(purchaseOrderId).stream().map(GoodsReceiptMapper::toResponse).toList(); }
    ...
    public GoodsReceiptResponse loadPurchaseOrderLines(UUID goodsReceiptId) {
        ...
        if (goodsReceipt.getStatus() != ReceiptStatus.DRAFT) throw new InvalidWorkflowException("Only draft Goods Receipts can load Purchase Order Lines.");
        PurchaseOrder purchaseOrder = goodsReceipt.getPurchaseOrder();
        List<PurchaseOrderLine> poLines = purchaseOrderLineRepository.findByPurchaseOrderId(purchaseOrder.getId()).stream().filter(line -> line.getOutstandingQuantity() != null && line.getOutstandingQuantity().compareTo(BigDecimal.ZERO) > 0).toList();
        if (poLines.isEmpty()) throw new ResourceNotFoundException("Purchase Order has no outstanding lines to receive.");
        if (goodsReceiptLineRepository.existsByGoodsReceiptId(goodsReceipt.getId())) throw new InvalidWorkflowException("Goods Receipt Lines have already been loaded.");
    ...
    public void delete(UUID id) {
        ...
        if (goodsReceipt.getStatus() != ReceiptStatus.DRAFT) throw new InvalidWorkflowException("Only draft Goods Receipts can be deleted.");
