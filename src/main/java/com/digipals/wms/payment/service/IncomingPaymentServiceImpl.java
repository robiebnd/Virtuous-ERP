...
public class IncomingPaymentServiceImpl implements IncomingPaymentService {
    ...
    public IncomingPayment receive(CreateIncomingPaymentRequest request) {
        BillingDocument billing = billingDocumentRepository.findById(request.billingDocumentId()).orElseThrow(() -> new EntityNotFoundException("Billing document not found: " + request.billingDocumentId()));
        if (billing.getStatus() != BillingStatus.POSTED) throw new IllegalStateException("Payment can only be received against a posted billing document");
        String paymentCurrency = request.currency().trim().toUpperCase(Locale.ROOT);
        if (billing.getCurrency() == null || !billing.getCurrency().equalsIgnoreCase(paymentCurrency)) throw new IllegalArgumentException("Payment currency must match billing document currency");
        BigDecimal paymentAmount = request.amount();
        if (paymentAmount == null || paymentAmount.compareTo(BigDecimal.ZERO) <= 0) throw new IllegalArgumentException("Payment amount must be greater than zero");
        BigDecimal previousApplied = allocationRepository.findActiveByBillingDocumentId(billing.getId(), PaymentStatus.CANCELLED).stream().map(PaymentAllocation::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        if (outstanding.compareTo(BigDecimal.ZERO) <= 0) throw new IllegalStateException("Billing document is already fully paid");
        BigDecimal appliedAmount = paymentAmount.min(outstanding);
        BigDecimal unappliedAmount = paymentAmount.subtract(appliedAmount);
        IncomingPayment payment = IncomingPayment.builder().paymentNumber(nextPaymentNumber()).customerCode(billing.getCustomerCode()).amount(paymentAmount).currency(paymentCurrency).paymentDate(LocalDateTime.now()).reference(normalizeReference(request.reference())).status(appliedAmount.compareTo(paymentAmount) == 0 ? PaymentStatus.FULLY_APPLIED : PaymentStatus.PARTIALLY_APPLIED).build();
        ...
        if (appliedAmount.compareTo(BigDecimal.ZERO) > 0) financePostingService.postIncomingPayment(saved.getId(), saved.getPaymentNumber(), saved.getCurrency(), appliedAmount);
        return saved;
    }
    ...
    @Override @Transactional(readOnly = true) public List<IncomingPayment> findByCustomerCode(String customerCode) { return paymentRepository.findByCustomerCodeOrderByPaymentDateDesc(customerCode); }
    private String normalizeReference(String reference) { if (reference == null) return null; String normalized = reference.trim(); return normalized.isEmpty() ? null : normalized; }
    private String nextPaymentNumber() { return "PAY-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase(Locale.ROOT); }
}
