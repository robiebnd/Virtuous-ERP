...
public class ProcurementClosureService {
    ...
    public Map<String,Object> reconcile(String poNumber){
        ...
        BigDecimal receivedQty=BigDecimal.ZERO,invoicedQty=BigDecimal.ZERO,orderedQty=BigDecimal.ZERO,receivedValue=BigDecimal.ZERO,invoicedValue=BigDecimal.ZERO;
        for(PurchaseOrderLine l:lines){
            BigDecimal ordered=nz(l.getQuantity()),received=nz(l.getReceivedQuantity()),inv=nz(invoiced.get(l.getId())),price=nz(l.getUnitPrice()); orderedQty=orderedQty.add(ordered);receivedQty=receivedQty.add(received);invoicedQty=invoicedQty.add(inv);receivedValue=receivedValue.add(received.multiply(price));
            BigDecimal lineInvoicedValue=invoiceLines.findByPurchaseOrderLineId(l.getId()).stream().map(x->nz(x.getLineTotal())).reduce(BigDecimal.ZERO,BigDecimal::add); invoicedValue=invoicedValue.add(lineInvoicedValue);
            Map<String,Object> row=new LinkedHashMap<>(); row.put("sku",l.getProduct().getSku());row.put("orderedQuantity",ordered);row.put("receivedQuantity",received);row.put("invoicedQuantity",inv);row.put("receivedValue",received.multiply(price));row.put("invoicedValue",lineInvoicedValue);row.put("uninvoicedReceivedQuantity",received.subtract(inv).max(BigDecimal.ZERO));row.put("openQuantity",ordered.subtract(received).max(BigDecimal.ZERO));row.put("quantityReconciled",received.compareTo(inv)==0);row.put("valueReconciled",received.multiply(price).compareTo(lineInvoicedValue)==0);detail.add(row);
        }
        ...
        Map<String,Object> result=new LinkedHashMap<>(); result.put("purchaseOrderNumber",po.getPoNumber());result.put("supplierCode",po.getSupplier().getCode());result.put("orderedQuantity",orderedQty);result.put("receivedQuantity",receivedQty);result.put("invoicedQuantity",invoicedQty);result.put("orderedValue",orderedQty.multiply(lines.isEmpty()?BigDecimal.ZERO:nz(lines.get(0).getUnitPrice())));result.put("receivedValue",receivedValue);result.put("invoicedValue",invoicedValue);result.put("uninvoicedReceivedQuantity",receivedQty.subtract(invoicedQty).max(BigDecimal.ZERO));result.put("invoiceBalanceDue",outstandingPayments);result.put("fullyReceived",receivedQty.compareTo(orderedQty)==0);result.put("fullyInvoiced",invoicedQty.compareTo(receivedQty)==0&&receivedValue.compareTo(invoicedValue)==0);result.put("fullyPaid",outstandingPayments.signum()==0&& !supplierInvoices.isEmpty());result.put("cycleReconciled",receivedQty.compareTo(orderedQty)==0&&invoicedQty.compareTo(receivedQty)==0&&receivedValue.compareTo(invoicedValue)==0&&outstandingPayments.signum()==0&&!supplierInvoices.isEmpty());result.put("lines",detail);return result;
    }

    public Map<String,Object> close(String poNumber){ PurchaseOrder po=po(poNumber); if(po.getStatus()==PurchaseOrderStatus.CLOSED) return Map.of("purchaseOrderNumber",po.getPoNumber(),"status",po.getStatus().name(),"message","Procurement cycle is already closed."); Map<String,Object> reconciliation=reconcile(poNumber); if(!Boolean.TRUE.equals(reconciliation.get("cycleReconciled"))) throw new InvalidWorkflowException("Purchase Order cannot be closed until receipt, invoice and payment quantities/values reconcile."); po.setStatus(PurchaseOrderStatus.CLOSED);purchaseOrders.save(po);return Map.of("purchaseOrderNumber",po.getPoNumber(),"status",po.getStatus().name(),"message","Procurement cycle closed successfully."); }
    ...
    private void validateScore(BigDecimal s,String name){if(s==null||s.compareTo(BigDecimal.ONE)<0||s.compareTo(BigDecimal.valueOf(100))>0)throw new InvalidWorkflowException(name+" must be between 1 and 100.");}
}
