...
public class ProcurementClosureService {
    ...
    public Map<String,Object> verifyInvoice(SupplierInvoiceRequest r) {
        ...
        if (po.getStatus() == PurchaseOrderStatus.CANCELLED || po.getStatus() == PurchaseOrderStatus.DRAFT)
            throw new InvalidWorkflowException("Only an approved or received Purchase Order can be invoiced.");
        ...
        Map<UUID,BigDecimal> alreadyInvoiced = alreadyInvoiced(po.getId());
        BigDecimal total = BigDecimal.ZERO;
        SupplierInvoice invoice = SupplierInvoice.builder()
                .invoiceNumber(r.supplierInvoiceNumber().trim())
                .supplier(po.getSupplier()).purchaseOrder(po)
                .invoiceDate(r.invoiceDate() == null ? LocalDateTime.now() : r.invoiceDate())
                .currency(r.currency() == null ? po.getCurrency() : r.currency().trim().toUpperCase())
                .status(SupplierInvoiceStatus.POSTED).subtotal(BigDecimal.ZERO).totalAmount(BigDecimal.ZERO)
                .amountPaid(BigDecimal.ZERO).balanceDue(BigDecimal.ZERO).build();

        for (InvoiceLineRequest lr : r.lines()) {
            if (lr.sku() == null || lr.quantity() == null || lr.unitPrice() == null || lr.quantity().signum() <= 0 || lr.unitPrice().signum() < 0)
                throw new InvalidWorkflowException("Invoice line requires a valid SKU, positive quantity and non-negative unit price.");
            ...
            BigDecimal received = nz(pol.getReceivedQuantity());
            BigDecimal prior = nz(alreadyInvoiced.get(pol.getId()));
            if (prior.add(lr.quantity()).compareTo(received) > 0)
                throw new InvalidWorkflowException("Invoice quantity exceeds received quantity for SKU " + lr.sku());
            if (lr.unitPrice().compareTo(nz(pol.getUnitPrice())) != 0)
                throw new InvalidWorkflowException("Invoice price does not match Purchase Order price for SKU " + lr.sku());
            ...
            alreadyInvoiced.put(pol.getId(), prior.add(lr.quantity()));
        }
        ...
        Map<String,Object> response = invoiceResponse(saved);
        response.put("threeWayMatch", "PASSED");
        response.put("matchBasis", "PURCHASE_ORDER + GOODS_RECEIPT + SUPPLIER_INVOICE");
        return response;
    }
    ...
    public Map<String,Object> pay(SupplierPaymentRequest r) {
        ...
        if (amount.signum() <= 0) throw new InvalidWorkflowException("Payment amount must be greater than zero.");
        if (amount.compareTo(nz(invoice.getBalanceDue())) > 0) throw new InvalidWorkflowException("Payment exceeds supplier invoice balance.");
        String number = "VPAY-" + UUID.randomUUID().toString().substring(0,8).toUpperCase();
        SupplierPayment payment = payments.save(SupplierPayment.builder().paymentNumber(number).supplier(invoice.getSupplier()).invoice(invoice).paymentDate(LocalDateTime.now()).amount(amount).paymentMethod(r.paymentMethod()).reference(r.reference()).status(SupplierPaymentStatus.CLEARED).build());
        invoice.setAmountPaid(nz(invoice.getAmountPaid()).add(amount));
        invoice.setBalanceDue(invoice.getTotalAmount().subtract(invoice.getAmountPaid()).max(BigDecimal.ZERO));
        invoice.setStatus(invoice.getBalanceDue().signum() == 0 ? SupplierInvoiceStatus.PAID : SupplierInvoiceStatus.PARTIALLY_PAID);
    ...
    public Map<String,Object> reconcile(String poNumber) {
        PurchaseOrder po = po(poNumber);
        List<PurchaseOrderLine> lines = poLines.findByPurchaseOrderId(po.getId());
        Map<UUID,BigDecimal> invoiced = alreadyInvoiced(po.getId());
        ...
        for (PurchaseOrderLine l : lines) {
            BigDecimal ordered=nz(l.getQuantity()), received=nz(l.getReceivedQuantity()), inv=nz(invoiced.get(l.getId()));
            orderedTotal=orderedTotal.add(ordered); receivedTotal=receivedTotal.add(received); invoicedTotal=invoicedTotal.add(inv);
            detail.add(new LinkedHashMap<>(Map.of("sku",l.getProduct().getSku(),"orderedQuantity",ordered,"receivedQuantity",received,"invoicedQuantity",inv,"uninvoicedReceivedQuantity",received.subtract(inv).max(BigDecimal.ZERO),"openQuantity",ordered.subtract(received).max(BigDecimal.ZERO),"quantityReconciled",received.compareTo(inv)==0)));
        }
        List<SupplierInvoice> supplierInvoices=invoices.findByPurchaseOrderId(po.getId());
        BigDecimal outstandingPayments=supplierInvoices.stream().map(i -> nz(i.getBalanceDue())).reduce(BigDecimal.ZERO,BigDecimal::add);
        return new LinkedHashMap<>(Map.of("purchaseOrderNumber",po.getPoNumber(),"supplierCode",po.getSupplier().getCode(),"orderedQuantity",orderedTotal,"receivedQuantity",receivedTotal,"invoicedQuantity",invoicedTotal,"uninvoicedReceivedQuantity",receivedTotal.subtract(invoicedTotal).max(BigDecimal.ZERO),"invoiceBalanceDue",outstandingPayments,"fullyReceived",receivedTotal.compareTo(orderedTotal)==0,"fullyInvoiced",invoicedTotal.compareTo(receivedTotal)==0,"fullyPaid",outstandingPayments.signum()==0,"cycleReconciled",receivedTotal.compareTo(orderedTotal)==0 && invoicedTotal.compareTo(receivedTotal)==0 && outstandingPayments.signum()==0,"lines",detail));
    }

    public Map<String,Object> close(String poNumber) {
        PurchaseOrder po = po(poNumber);
        Map<String,Object> reconciliation = reconcile(poNumber);
        if (!Boolean.TRUE.equals(reconciliation.get("cycleReconciled"))) throw new InvalidWorkflowException("Purchase Order cannot be closed until receipt, invoice and payment quantities/values reconcile.");
        po.setStatus(PurchaseOrderStatus.CLOSED); purchaseOrders.save(po);
        return Map.of("purchaseOrderNumber",po.getPoNumber(),"status",po.getStatus().name(),"message","Procurement cycle closed successfully.");
    }

    public Map<String,Object> evaluate(VendorEvaluationRequest r) {
        PurchaseOrder po = r.purchaseOrderNumber() == null || r.purchaseOrderNumber().isBlank() ? null : po(r.purchaseOrderNumber());
        Supplier supplier = po != null ? po.getSupplier() : null;
        if (supplier == null) throw new InvalidWorkflowException("Purchase Order number is required for vendor evaluation.");
        validateScore(r.priceScore(),"priceScore"); validateScore(r.qualityScore(),"qualityScore"); validateScore(r.deliveryScore(),"deliveryScore"); validateScore(r.serviceScore(),"serviceScore");
        BigDecimal overall=nz(r.priceScore()).add(nz(r.qualityScore())).add(nz(r.deliveryScore())).add(nz(r.serviceScore())).divide(BigDecimal.valueOf(4),2,RoundingMode.HALF_UP);
        VendorEvaluation e=evaluations.save(VendorEvaluation.builder().supplier(supplier).purchaseOrder(po).priceScore(r.priceScore()).qualityScore(r.qualityScore()).deliveryScore(r.deliveryScore()).serviceScore(r.serviceScore()).overallScore(overall).evaluationDate(LocalDateTime.now()).remarks(r.remarks()).build());
        return Map.of("supplierCode",supplier.getCode(),"supplierName",supplier.getName(),"purchaseOrderNumber",po.getPoNumber(),"priceScore",e.getPriceScore(),"qualityScore",e.getQualityScore(),"deliveryScore",e.getDeliveryScore(),"serviceScore",e.getServiceScore(),"overallScore",e.getOverallScore());
    }
    ...
    private Map<String,Object> invoiceResponse(SupplierInvoice i) { return new LinkedHashMap<>(Map.of("invoiceNumber",i.getInvoiceNumber(),"purchaseOrderNumber",i.getPurchaseOrder().getPoNumber(),"supplierCode",i.getSupplier().getCode(),"status",i.getStatus().name(),"totalAmount",i.getTotalAmount(),"balanceDue",i.getBalanceDue())); }
