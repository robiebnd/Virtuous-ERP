...
public class PurchaseRequisitionController {
    ...
    public PurchaseRequisitionResponse findByNumber(@PathVariable String requisitionNumber) {
        ...
        return service.findAll().stream().filter(requisition -> normalizedNumber.equalsIgnoreCase(requisition.getRequisitionNumber())).findFirst()
    ...
    public ResponseEntity<byte[]> pdfByNumber(@PathVariable String requisitionNumber) { return pdfResponse(pdfService.generateByNumber(requisitionNumber), requisitionNumber + ".pdf"); }
    @GetMapping("/{id}/pdf")
    public ResponseEntity<byte[]> pdf(@PathVariable UUID id) { return pdfResponse(pdfService.generateById(id), "purchase-requisition.pdf"); }
    ...
    public ResponseEntity<Void> clearLines(@PathVariable UUID id) {
        service.clearLines(id);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/{requisitionId}/import-quotation/{quotationId}")
    public PurchaseRequisitionResponse importQuotation(@PathVariable UUID requisitionId, @PathVariable UUID quotationId) { return service.importQuotation(requisitionId, quotationId); }
    @PostMapping("/{requisitionId}/import-quotation/number/{quotationNumber}")
    public PurchaseRequisitionResponse importQuotationByNumber(@PathVariable UUID requisitionId, @PathVariable String quotationNumber) { return service.importQuotationByNumber(requisitionId, quotationNumber); }
