...
public class PurchaseRequisitionServiceImpl implements PurchaseRequisitionService {
    ...
    private PurchaseRequisitionResponse toResponseWithLines(PurchaseRequisition requisition) {
        ...
        List<PurchaseRequisitionResponse.LineResponse> lines = lineRepository.findByPurchaseRequisitionId(requisition.getId()).stream().map(line -> {
            ...
            return PurchaseRequisitionResponse.LineResponse.builder().id(line.getId()).productId(line.getProduct().getId()).sku(line.getProduct().getSku()).productName(line.getProduct().getName()).quantity(line.getQuantity()).estimatedUnitCost(unitCost).estimatedLineTotal(lineTotal).remarks(line.getRemarks()).build();
        }).toList();
    ...
    @Override public PurchaseRequisitionResponse create(CreatePurchaseRequisitionRequest request) { Warehouse warehouse = getWarehouse(request.getWarehouseId()); Supplier supplier = getSupplier(request.getSupplierId()); PurchaseRequisition requisition = PurchaseRequisition.builder().requisitionNumber(documentNumberService.next(DocumentType.PURCHASE_REQUISITION)).warehouse(warehouse).supplier(supplier).department(request.getDepartment().trim()).currency(normalizeCurrency(request.getCurrency())).remarks(request.getRemarks()).status(PurchaseRequisitionStatus.DRAFT).requestedBy(currentUserService.getCurrentUser()).build(); return toResponseWithLines(repository.save(requisition)); }
    @Override public PurchaseRequisitionResponse update(UUID id, UpdatePurchaseRequisitionRequest request) { PurchaseRequisition requisition = getRequisition(id); validator.validateDraft(requisition); requisition.setSupplier(getSupplier(request.getSupplierId())); requisition.setDepartment(request.getDepartment().trim()); requisition.setCurrency(normalizeCurrency(request.getCurrency())); requisition.setRemarks(request.getRemarks()); return toResponseWithLines(repository.save(requisition)); }
    ...
    @Override public PurchaseRequisitionResponse importQuotation(UUID requisitionId, UUID quotationId) {
